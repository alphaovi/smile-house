...
import { Calendar, Filter, Building2, UserCheck, Search, MapPin, DollarSign } from 'lucide-react';
...
const FilterSection = ({
  ...
  // Doctor বেছে নিলে Clinic ও State অটো-সিলেক্ট হবে
  const handleDoctorChange = (docId) => {
    setSelectedDoctor(docId);
    if (docId !== 'All') {
      const matchDoc = (reportData || []).find((item) => String(item.doctorId) === String(docId));
      if (matchDoc) {
  ...
  // Clinic বেছে নিলে Doctor ও State অটো-সিলেক্ট হবে
  const handleClinicChange = (clinicId) => {
    setSelectedClinic(clinicId);
    if (clinicId !== 'All') {
      const matchClinic = (reportData || []).find((item) => String(item.clinicId) === String(clinicId));
      if (matchClinic) {
  ...
  // State চেঞ্জ করলে Doctor ও Clinic রিসেট হবে
  ...
  return (
    <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
      ...
      <div className="flex justify-end pt-2">
        ...
        >
          <Search className="w-4 h-4" /> Load Client Statement
        </button>
      </div>
    </div>
  );
